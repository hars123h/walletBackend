...
exports.team_sum = async(req, res) => {
  const {user_id} = req.body;
  const user_data =  await User.findById(user_id).then(response=>response);
  var l1 = [], l2 = [], l3 = [];
  user_data.directMember.map(async(member_id)=>{
    const member_data = await User.findById(member_id).then(response=>response);
    l1.push({
      mobno:member_data.mobno,
      totalRecharge: member_data.recharge_amount
    })
  });

  user_data.indirectMember.map(async(member_id)=>{
    const member_data = await User.findById(member_id).then(response=>response);
    l2.push({
      mobno:member_data.mobno,
      totalRecharge: member_data.recharge_amount
  ...
  user_data.in_indirectMember.map(async(member_id)=>{
    const member_data = await User.findById(member_id).then(response=>response);
    l3.push({
      mobno:member_data.mobno,
      totalRecharge: member_data.recharge_amount
  ...
  try {
    res.status(200).json({level1:l1, level2:l2, level3:l3});
  } catch (error) {
    res.status(400).json({message:'Something went wrong!'});
  }
}